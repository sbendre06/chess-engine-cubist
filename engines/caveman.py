...
def get_pseudo_legal_moves(
    ...
    """Return all pseudo-legal moves, captures listed before quiet moves.

    Args:
        board: chess.variant.AntichessBoard — current position.

    Returns:
        list[chess.Move] — pseudo-legal candidates with all captures preceding
        all quiet moves.  The harness filters this list through board.legal_moves
        (which enforces the mandatory-capture rule) before any move is played.

    Antichess:
        Returns pseudo-legals rather than legals, so quiet moves are included
        even when a capture exists; those quiet moves are silently discarded by
        the harness.  Captures appear first so the post-filter result is already
        capture-ordered when captures are mandatory.
    """
...
def evaluate_board(
    ...
    Args:
        board: chess.variant.AntichessBoard — current position; board.turn
            identifies which side is evaluated (us/them).

    Returns:
        int — side-to-move-positive score.  Components (via bitboard ops):
        -own_badness + opp_badness//5 (inverted piece-type burden), +60 per own
        piece currently attacked by the opponent (capture-liability bonus), +20
        per opponent piece we currently threaten (attack-pressure bonus), +40 per
        piece we are below 16 (raw count bonus), -10 per piece opponent is below
        16.  Terminal positions are scored by the harness with MATE_SCORE.

    Antichess:
        _BADNESS encodes how hard each piece type is to sacrifice: queens and
        bishops are highest (hardest to lose), pawns and kings lowest.  Capture
        liability (own pieces the opponent can take) is the key positional signal;
        own pieces en prise will soon be removed from our inventory, which is the
        win condition.
    """
    us = board.turn
    them = not us
...
def order_moves(
    ...
    """Reorder moves: opening book first, then promotions, captures (sacrifice-weighted), quiet pawn pushes.

    Args:
        board: chess.variant.AntichessBoard — current position; used to
            look up the opening book and to classify and score each move.
        moves: list[chess.Move] — filtered legal moves to reorder.

    Returns:
        list[chess.Move] — same moves in preferred search order.  If a book
        move is available it is placed first; remaining moves are sorted as:
        promotions (rook first per Watkins' proof, then king, knight, others),
        captures (prefer sacrificing high-burden own pieces), quiet pawn pushes
        (advance toward promotion rank).

    Antichess:
        Capture scoring uses own-piece _BADNESS rather than captured-piece value:
        we want our stickiest pieces (high _BADNESS) to be the capturing piece so
        they leave our inventory.  Rook-first promotion follows Watkins' finding
        that rook promotions create better endgame piece structures than queens or
        bishops, which are harder to subsequently sacrifice.
    """
    # Check opening book.
    key = _pos_key(board)
